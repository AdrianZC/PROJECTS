...
const FRICTION = 1; // friction coefficient of space (0 = no friction, 1 = lots of friction)
...
const SHIP_INV_DUR = 2; // duration of the ship's invisibility in seconds
...
function update() {
    ...
    for (var i = ship.lasers.length - 1; i >= 0; i--) {
        ...
        // handle edge of screen
        if (ship.lasers[i].x < 0) { // If laser goes off the left edge
            ship.lasers.splice(i, 1); // Remove the laser

        } else if (ship.lasers[i].x > canv.width) { // If laser goes off the right edge
            ship.lasers.splice(i, 1); // Remove the laser

        }
        if (ship.lasers[i].y < 0) { // If laser goes off the top edge
            ship.lasers.splice(i, 1); // Remove the laser

        } else if (ship.lasers[i].y > canv.height) { // If laser goes off the bottom edge
            ship.lasers.splice(i, 1); // Remove the laser
